...
var aspect =
  Calc.graphpaperBounds.mathCoordinates.width /
  Calc.graphpaperBounds.mathCoordinates.height
aspect = aspect ** -1
...
var menuAnimation = setInterval(function () {
  if (inMenu) {
    ...
    Calc.setExpression({
      id: '5',
      latex: 'a=' + Math.round(animation * 1000) / 1000
    })
    cameraAnimator += 0.01
    Calc.setMathBounds({
      left: -6 + Math.sin(cameraAnimator) / 3,
      right: 6 + Math.sin(cameraAnimator) / 3,
      bottom: (-6 + Math.sin(cameraAnimator * Math.SQRT1_2) / 3) * aspect,
      top: (6 + Math.sin(cameraAnimator * Math.SQRT1_2) / 3) * aspect
    })
    Calc.setExpression({
      id: ['12', '14', '13'][menuSelection],
      color: Calc.colors.ORANGE
    })
    Calc.setExpression({
      id: ['12', '14', '13'][(menuSelection + 1) % 3],
      color: Calc.colors.BLACK
    })
    Calc.setExpression({
      id: ['12', '14', '13'][(menuSelection + 2) % 3],
      color: Calc.colors.BLACK
    })
  }
}, 1000 / 60)
var circularInterval = setInterval(function () {
  if (inGame && gameIn == 1) {
    ...
    Calc.setMathBounds({
      left: -6 + Math.cos(circularChar.p) / 2,
      right: 6 + Math.cos(circularChar.p) / 2,
      bottom: -6 * aspect + Math.sin(circularChar.p) / 2,
      top: 6 * aspect + Math.sin(circularChar.p) / 2
    })
    ...
    if (circularPosition.x ** 2 + circularPosition.y ** 2 > 0.95) {
      if (
        Math.abs(
          ((Math.atan2(circularPosition.y, circularPosition.x) + 2 * Math.PI) %
            (2 * Math.PI)) -
            (((circularChar.p % (2 * Math.PI)) + 2 * Math.PI) % (2 * Math.PI))
        ) > 0.225 &&
        Math.abs(
          ((Math.atan2(circularPosition.y, circularPosition.x) + 2 * Math.PI) %
            (2 * Math.PI)) -
        ...
        gotoMenu()
      circularDirection =
        Math.atan2(circularPosition.y, circularPosition.x) +
        Math.PI +
        Math.random() -
        0.5
    ...
    if (inGame)
      Calc.setExpression({ id: '6', latex: 'b_{x}=' + circularPosition.x })
    if (inGame)
      Calc.setExpression({ id: '7', latex: 'b_{y}=' + circularPosition.y })
    gameSpeed += 0.01
  }
}, 1000 / 60)
var oneInterval = setInterval(function () {
  if (inGame && gameIn == 2) {
    Calc.setExpression({
      id: '1',
      latex: 'p_{1}=' + Math.round(player1 * 1000) / 1000
    })
    Calc.setExpression({
      id: '9',
      latex: 'b_{x}=' + Math.round(ball.x * 1000) / 1000
    })
    Calc.setExpression({
      id: '10',
      latex: 'b_{y}=' + Math.round(ball.y * 1000) / 1000
    })
...
var twoInterval = setInterval(function () {
  if (inGame && gameIn == 0) {
    ...
    Calc.setExpression({
      id: '2',
      latex: 'p_{2}=' + Math.round(player2 * 1000) / 1000
    })
    Calc.setExpression({
      id: '9',
      latex: 'b_{x}=' + Math.round(ball.x * 1000) / 1000
